...
@section('my_body')
@parent
<Divider orientation="left">加班确认信息</Divider>

<Tabs type="card" v-model="currenttabs">

	<Tab-pane Icon="ios-list-box-outline" label="确认列表">
			...
			<Panel name="1">
				...
				<p slot="content">
					...
					<i-row :gutter="16">
						...
						<i-col span="8">
							提交时间&nbsp;
							<Date-picker v-model.lazy="queryfilter_created_at" @on-change="jiabangetsconfirm(page_current, page_last)" type="datetimerange" format="yyyy-MM-dd HH:mm" size="small" placeholder="" style="width:250px"></Date-picker>
						</i-col>
						<i-col span="2">
							@hasanyrole('role_super_admin')
								<Checkbox v-model="queryfilter_trashed" @on-change="jiabangetsconfirm(page_current, page_last)">已删除</Checkbox>
							@else
								&nbsp;
							@endhasanyrole
						</i-col>
...
var vm_app = new Vue({
	...
	methods: {
		...
		// 生成piliangluru_confirm
		piliangluru_confirm_generate(counts) {
			if (counts == undefined) counts = 1;
			var len = this.piliangluru_confirm.length;
			
			if (counts > len) {
				for (var i=0;i<counts-len;i++) {
					// this.piliangluru_confirm.push({value: 'piliangluru_applicant'+parseInt(len+i+1)});
					this.piliangluru_confirm.push(
						{
                            uid: '',
                            applicant: '',
                            department: '',
				...
				if (this.piliangluruxiang_applicant2 != '') {
					for (var i=counts;i<len;i++) {
						if (this.piliangluruxiang_applicant2 == this.piliangluru_confirm[i].value) {
		...
		oncurrentpagechange: function (currentpage) {
			this.jiabangetsconfirm(currentpage, this.page_last);
		},
		...
		onpagesizechange: function (pagesize) {
			...
			.then(function (response) {
				...
				if (response.data) {
					...
					_this.jiabangetsconfirm(1, _this.page_last);
				} else {
		...
		jiabangetsconfirm(page, last_page){
			...
			var url = "{{ route('renshi.jiaban.jiabangetsconfirm') }}";
		...
		oncreate_applicant1: function () {
			...
			.then(function (response) {
				...
				if (response.data) {
					...
					_this.jiabangetsconfirm(_this.page_current, _this.page_last);
...
// console.log(_this.jiaban_edit_application);
// return false;
	...
	mounted: function(){
		var _this = this;
		_this.current_nav = '加班确认';
		_this.current_subnav = '确认';
		// 显示所有
		_this.jiabangetsconfirm(1, 1); // page: 1, last_page: 1
		_this.loadapplicantgroup();
